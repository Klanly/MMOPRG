...
public class WindowUIMgr : Singleton<WindowUIMgr>
{
    ...
    /// 开始打开窗口
    /// </summary>
    /// <param name="windowBase">窗口预设</param>
    ...
    private void ShowCenterToBig(UIWindowBase windowBase, bool isOpen)
    {
        ...
        ts.to = Vector3.one;
        ts.duration = windowBase.duration;//过程所需要时间
        ts.SetOnFinished(() =>//结束事件
        {
            if (!isOpen)
                DestroyWindow(windowBase);
        });
    ...
    private void ShowFromDir(UIWindowBase windowBase, int dirType, bool isOpen)
    {
        ...
        tp.to = Vector3.one;
        tp.duration = windowBase.duration;//过程所需要时间
        tp.SetOnFinished(() =>//结束事件
        {
            if (!isOpen)
                DestroyWindow(windowBase);
        });
